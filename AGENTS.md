...
- `_site/` = build output — **never edit by hand**. `docs/`, `design-export/`, `functions/`,
  `scripts/`, `.Codex/` are internal and excluded from the build via `.eleventyignore`.
...
(`.Codex/skills/verify-coreflow/`) — it builds and runs 8 checks against `_site/` and outputs a
