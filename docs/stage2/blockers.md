...
- **Fax cover sheet PDF — SHIPPED (corrected version, Pass A.1).** The first vendor PDF printed the wrong fax `(854) 209-2494` and was withheld; a second one printed the correct fax but had a **layout defect** (physician-order box overlapping the confidentiality notice — invisible to text extraction). The corrected file was shipped, then on 2026-09-03 replaced with the **branded "final"** cover sheet (CoreFlow Rx logo/header), pinned by SHA-256 `7598d9373bae76e22d994191e3854f885a64a8561030482b9ba879c87bd76012`, wired to all four download buttons. Guarded by verify-coreflow **Check 12** (hash pin primary + geometry secondary; rendered to confirm no overlap).
