...
3. **workers.dev alias.** Access on the custom domain does **not** protect a `*.workers.dev` URL. If `coreflowrx.john-057.workers.dev` is enabled, it is an unprotected bypass of Access. **Confirm in the dashboard whether that alias is enabled and disable it** (Workers & Pages → coreflowrx → Settings → Domains & Routes → disable the workers.dev route). The agent cannot verify this from the repo — **UNRESOLVED, needs your check.**
